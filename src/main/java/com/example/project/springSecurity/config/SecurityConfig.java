package com.example.project.springSecurity.config;
...
import com.example.project.springSecurity.service.MyOAuth2UserService;
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf(auth -> auth.disable())			// 괄호 안에 람다함수를 사용해야 함
			...
			.formLogin(auth -> auth
				...
				.loginProcessingUrl("/user/login") // SpringSecurity가 낚아 챔. *UserDetailsService 구현객체에서 처리해주어야 함.
				.usernameParameter("uid")
				.passwordParameter("pwd")
				.defaultSuccessUrl("/user/loginSuccess", true)		// 내가 로그인후 해야할 일, 예) 세션 세팅, 오늘의 메세지
				.permitAll()
			)
